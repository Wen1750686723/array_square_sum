...
if test "$PHP_ARRAY_SQUARE_SUM" != "no"; then
  ...
  dnl [
  dnl   PHP_ADD_LIBRARY_WITH_PATH($LIBNAME, $ARRAY_SQUARE_SUM_DIR/lib, ARRAY_SQUARE_SUM_SHARED_LIBADD)
  dnl   AC_DEFINE(HAVE_ARRAY_SQUARE_SUMLIB,1,[ ])
  dnl ],[
  dnl   AC_MSG_ERROR([wrong array_square_sum lib version or lib not found])
  dnl ],[
  dnl   -L$ARRAY_SQUARE_SUM_DIR/lib -lm
  dnl ])
  dnl
  dnl PHP_SUBST(ARRAY_SQUARE_SUM_SHARED_LIBADD)

  PHP_NEW_EXTENSION(array_square_sum, array_square_sum.c, $ext_shared)
fi
